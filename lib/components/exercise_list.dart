            ...
            return Padding(
              ...
              child: Column(
                children: [
                  GestureDetector(
                    onTap: () async {
                      if(cubit.user!.isAnonymous){
                        Navigator.push(
                            context,
                            MaterialPageRoute(
                                builder: (context) => SingleExerciseScreen(
                                    index: index,
                                    exerciseList: cubit.exerciseList)));
                      }else if(cubit.user != null){
                        await ExerciseCubit.get(context)
                            .checkExercise(cubit.exerciseList?[index].name);
                          ...
                          children: [
                            Flexible(
                              child: Column(
                                ...
                                children: [
                                  Text(
                                    "Exercise Name: ${cubit.exerciseList?[index].name}",
                                    style: GoogleFonts.itim(),
                                    overflow: TextOverflow.fade,
                                  ),
                                  Text(
                                      "Body Part: ${cubit.exerciseList?[index].bodyPart}",
                                      style: GoogleFonts.itim()),
                                  Text(
                                      "Targets: ${cubit.exerciseList?[index].target}",
                                      style: GoogleFonts.itim()),
                                ],
                              ),
                            ),
                            ClipRRect(
                                ...
                                child: Image(
                                    image: NetworkImage(
                                        "${cubit.exerciseList?[index].gifUrl}")))
                          ],
                        ),
                      ),
                    ),
